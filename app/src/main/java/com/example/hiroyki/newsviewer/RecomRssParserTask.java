...
// RssParserTask.java
public class RecomRssParserTask extends AsyncTask<String, Integer, ItemListAdapter2> {
    public RecomFragment mActivity;
    private ItemListAdapter2 mAdapter;
    private ProgressDialog mProgressDialog;

    // コンストラクタ
    public RecomRssParserTask(RecomFragment activity, ItemListAdapter2 adapter) {
        mActivity = activity;
        mAdapter = adapter;
    }
    ...
    @Override
    protected ItemListAdapter2 doInBackground(String... params) {
        ItemListAdapter2 result = null;
        try {
    ...
    // メインスレッド上で実行される
    @Override
    protected void onPostExecute(ItemListAdapter2 result) {
        mProgressDialog.dismiss();
        mActivity.setListAdapter(result);
    }

    // XMLをパースする
    public ItemListAdapter2 parseXml(InputStream is) throws IOException, XmlPullParserException {
